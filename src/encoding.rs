...
/// 
pub fn deserialize_match_result(buf: &[u8]) -> Result<Trade, &'static str> {
    if buf.len() < MESSAGE_TOTAL_SIZE {
        return Err("Buffer size is too small for Trade.");
    }
    //println!("received hex: {}",to_hex(buf));
...
fn instance_tag_to_str(tag: &[u8; 16]) -> &str {
    let len = tag.iter().position(|&b| b == 0).unwrap_or(16);
    str::from_utf8(&tag[..len]).unwrap_or("<invalid-utf8>")
}
...
pub fn decode_broadcast_message(buf: &[u8]) -> Result<String, String> {
    ...
    match msg_type {
        MSG_ORDER EXECUTION_BROADCAST => {
            ...
            Ok(format!("🔥 ORDER EXECUTION: Product={} | Price={} | Qty={} | BuyOrderID={} | SellOrderId={}| Net={}ns | MatchCore={}ns", 
